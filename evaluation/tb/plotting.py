...
def get_scalar_dataframe_from_tb_files(log_dirs):
    ...
    def rename_axis(df):
        return df.rename(
            columns={"Environment/Cumulative Reward": "Mean Reward",
                     "step": "Timestep",
...
def plot_multi_series(data, figsize, title="Mean reward for last experience buffer", x_axis="Timestep", y_axis="Mean Reward"):
    # Make a data frame
    ...
    # handles = list()
    for name, values in data.items():
        subplot = sns.lineplot(x=x_axis, y=y_axis, data=values.drop_duplicates().reset_index(drop=True), label=name)
...
def plot_histogram(df, steps_to_plot=None, title="Distribution of cumulative episode reward per training session"):
    # Set background
    ...
    g = sns.FacetGrid(df, row='Timestep', hue='Timestep', aspect=15, height=.5, palette=pal)

    def plot_subplots(x, color, label, data):
        ax = plt.gca()
        ax.text(-0.05, .08, label, fontweight="bold", color=color,
        ...
        x, y = SummaryReader.histogram_to_bins(counts, limits, lower_bound=-3, upper_bound=3, n_bins=50)
